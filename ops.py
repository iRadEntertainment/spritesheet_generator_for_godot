...
class SGG_OT_plan_and_run(Operator):
    ...
    def invoke(self, context: Context, event: Event):
        ...
        # Set default camera to the scene camera, if any
    ...
    def _build_plan(self, scene: Scene, settings: SGG_GlobalSettings) -> None:
        ...
        for obj in armatures:
            ...
            for action in actions:
                ...
                act_item.enabled = True
                act_item.name = action.name
                act_item.action = action

                start, end = core.compute_action_frame_range(action)
    ...
    def draw(self, context: Context) -> None:
        ...
        if not plan:
            ...
            for arm_item in plan:
                # Armature row (checkbox + collapse triangle + name)
                ...
                if arm_item.ui_expanded:
                    for act_item in arm_item.actions:
                        ...
                        act_row.prop(act_item, "enabled", text="")
                        act_row.label(text=act_item.name, icon="ACTION")
        ...
        summary_box.label(text=f"Spritesheets (approx): {total_sheets}")
        summary_box.label(text="Estimated time: —")  # will use history later
